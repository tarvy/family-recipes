...
export async function POST(request: Request): Promise<Response> {
  return withRequestContext(request, () =>
    withTrace('api.admin.magic-link.create', (span) => handlePost(request, span)),
  );
}

async function handlePost(request: Request, span: MinimalSpan): Promise<Response> {
  const user = await getSessionFromCookies(await cookies());
  const authorizationResponse = authorizeOwner(user, span);
  ...
  try {
    ...
    return createResponse(email, user?.id ?? '', span);
  } catch (error) {
...
function authorizeOwner(
  user: Awaited<ReturnType<typeof getSessionFromCookies>>,
  span: MinimalSpan,
): Response | null {
  if (!user) {
...
async function createResponse(
  ...
  if (!(result.success && result.url && result.expiresAt)) {
    ...
    return Response.json({ error: 'generation_failed' }, { status: HTTP_INTERNAL_SERVER_ERROR });
  }
