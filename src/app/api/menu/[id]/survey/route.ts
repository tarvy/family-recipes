/** POST + DELETE /api/menu/[id]/survey */
...
import { cancelSurvey, MenuError, menuErrorHttpStatus, sendSurvey } from '@/lib/menu/service';
...
export async function POST(request: Request, { params }: RouteParams): Promise<Response> {
  return withRequestContext(request, () =>
    withTrace('api.menu.sendSurvey', async (span) => {
      ...
      try {
        const result = await sendSurvey(id);
        ...
        logger.api.error('Failed to send survey', toError(error));
...
export async function DELETE(request: Request, { params }: RouteParams): Promise<Response> {
  return withRequestContext(request, () =>
    withTrace('api.menu.cancelSurvey', async (span) => {
      const { id } = await params;
      span.setAttribute('menu_id', id);
      ...
      try {
        ...
        logger.api.error('Failed to cancel survey', toError(error));
        span.setAttribute('error', toErrorMessage(error));
        return Response.json(
          { error: 'Failed to cancel survey' },
          { status: HTTP_INTERNAL_SERVER_ERROR },
        );
      }
    }),
